...
export async function GET(request: NextRequest) {
  try {
    ...
    // Determine filename
    let filename = 'player-statistics'
    if (playerId) {
      const player = exportData[0]?.playerName
      filename = `${player?.toLowerCase().replace(/\s+/g, '-')}-statistics`
    } else if (seasonId) {
      const season = playerStats[0]?.season?.name
      filename = `player-statistics-${season}`
    } else {
      filename = 'player-statistics-all-time'
    }
    filename += '.csv'
    ...
    console.error('Error exporting player statistics:', error)
    return NextResponse.json(
      { error: 'Failed to export player statistics' },
      { status: 500 }
    )
  }
}
